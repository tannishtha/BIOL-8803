...
GetOptions('i1=s' => \$file1,
	...
	't=s' =>  \$threadCount,
	'o=s' => \$output,
) or die "Usage: $0 -i1 <file1> -i2 <file2> -m <percent Overlap> -t <Thread Count> -j -k <condition> -o <output file> {j and k are optional}\n";
...
foreach $key (sort keys %file1Start){
	...
	while(1){
		...
		my @joinable = threads->list(threads::joinable);
		$_->join() for(@joinable);
		last unless (@running >= $threadCount);
		sleep(1);
	}	
}
